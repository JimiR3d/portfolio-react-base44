  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      await sql`
        ...
        VALUES (${payload.name}, ${payload.description}, ${payload.techTags || []}, ${payload.githubUrl || null}, ${payload.liveUrl || null}, ${payload.status || 'ACTIVE'}, ${payload.displayOrder || 0}, ${payload.visible ?? true})
      `;
    ...
    if (req.method === 'PUT') {
      ...
      await sql`
        ...
        SET name = ${payload.name}, description = ${payload.description}, tech_tags = ${payload.techTags || []}, 
            github_url = ${payload.githubUrl || null}, live_url = ${payload.liveUrl || null}, status = ${payload.status || 'ACTIVE'}, display_order = ${payload.displayOrder}, visible = ${payload.visible},
            updated_at = NOW()
        WHERE id = ${id}
      `;
      return res.status(200).json({ ok: true });
